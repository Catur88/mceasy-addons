...
def terbilang(number, currency, bhs):
    ...
    # uswa-tambah ini untuk menghilangkan minusnya field 'amount_total'
    start_word = english_number(abs(int(lis[0])), bhs)
...
class AccountInvoice(models.Model):
    ...
    x_jatuh_tempo = fields.Date(string="Jatuh Tempo")
    x_bayaran_diterima = fields.Monetary(string="Bayaran Diterima", store=True, readonly=True,
                                     compute="_get_bayaran_diterima")
    # x_sisa_tagihan = fields.Monetary(string="Sisa Tagihan")
    amount_to_text = fields.Text(string="Terbilang", store=True, readonly=True,
                                 compute='_amount_in_word')

    @api.one
    @api.depends('residual')
    def _get_bayaran_diterima(self):
        inv_name = self.number
        acc_move_obj = self.env['account.payment'].search([('communication', '=', inv_name)])
        if acc_move_obj:
            x = 0
            for row in acc_move_obj:
                bayaran = row.amount
                x += bayaran

            self.x_bayaran_diterima = x


    @api.one
    def _get_sisa_tagihan(self):
        self.x_sisa_tagihan = self.x_bayaran_diterima - self.amount_total
    ...
    def _amount_in_word(self):
        if self.amount_total:
            ...
            self.amount_to_text = terbilang(self.amount_total, 'idr', 'id')
    ...
    def invoice_print_custom(self):
        ...
        self.ensure_one()
        self.sent = True
        return self.env['report'].get_action(self, 'lpj_sales.report_invoice_custom')
